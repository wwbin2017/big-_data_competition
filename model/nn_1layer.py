...
def build_graph(batch_size, data, act_val, data_shuffle, file_name_predict):
    ...
    with graph.as_default():
        ...
        # y_1_dropout = tf.nn.dropout(y_1_act, keep_prob=keep)
        '''
        # 第二层
        ...
        # 正则化
        reguralization = tf.nn.l2_loss(weights_1) + tf.nn.l2_loss(weights_4)  # + tf.norm(weights_3, 2) + tf.norm(weights_4, 2)
        ...
        # 损失函数
        loss = -tf.reduce_mean(train_labels*tf.log(y_process)) + 5e-2*reguralization
        ...
        with tf.Session() as sess:
            ...
            for step in range(100000):
                ...
                _, l, ac = sess.run([train_step, loss, accuracy],
                                    feed_dict={train_inputs: inputs,
                                               train_labels: labels,
                                               learning_rate: learn_rate*pow(0.99, step/100)})
                ...
                if step % 1000 == 0:
                    ...
                    print "运行步数： ",step, " 损失值: ", l, "  测试损失", l_test,"  权重 " , 5e-2*reg, \
                        " 学习率 ", learn_rate
            ...
            # print time_list
            file_name = "../result/2017_07_01_01.csv"
...
def main():
    # 训练集
    data, act_val = read_file("../data/bdc_v71/train_vec_v71.csv")
    # 预测集
    file_name_predict = "../data/bdc_v71/test_vec_v71.csv"
    batch_size = 128
